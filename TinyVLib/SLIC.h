...
public:
	...
	// [9/14/2015 Yinlin.Hu]
	void OverSegmentaion(FImage& img, int*& kLabels, const int K, const double compactness);
...
void SLIC::OverSegmentaion(FImage& img, int*& kLabels, const int K, const double compactness)
{
	int w = img.width();
	int h = img.height();
	int numLabels = 0;
	const int superpixelsize = 0.5 + double(w*h) / double(K);
	...
	DoSuperpixelSegmentation_ForGivenSuperpixelSize(buff, w, h, kLabels, numLabels, superpixelsize, compactness);
	delete[] buff;
}
	...
	const unsigned int&		color)
{
	...
	{
		int idx = contoury[i] * width + contourx[i];
		ubuff[idx] &= 0x60ffffff;

		/*
		for( int k = 0; k < 8; k++ )
		{
		int x = contourx[i] + dx8[k];
		int y = contoury[i] + dy8[k];
		if( (x >= 0 && x < width) && (y >= 0 && y < height) )
		{
		idx = y*width + x;
		if(!istaken[idx]) ubuff[idx] = 0;
		}
		}
		*/
	}
