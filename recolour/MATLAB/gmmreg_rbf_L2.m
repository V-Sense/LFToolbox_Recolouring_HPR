...
[~,d] = size(config.model); % number of points in model set
if (d~=2)&&(d~=3)
...
if(config.iter >1)
    ...
    options = optimset(  'LargeScale','off','GradObj','on', 'TolFun',1e-10, 'TolX',1e-10, 'TolCon', 1e-10);
end
...
%        Pn = [ones(n,1) config.ctrl_pts];
       ...
       [ param,~,~,~,~,~]  = fminunc(@(x)gmmreg_L2_rbf_costfunc(x, init_affine, basis, scene, scale, alpha, beta, n, d, K), x0,  options);
