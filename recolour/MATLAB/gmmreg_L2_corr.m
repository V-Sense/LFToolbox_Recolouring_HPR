...
if(config.iter >1)
    options = optimset(  'LargeScale','off','GradObj','on', 'TolFun',1e-5, 'TolX',1e-5, 'TolCon', 1e-5, 'MaxIter',  config.max_iter);%,'display', 'iter'); % not really used as iterations used to stop optisation in this case
else
    options = optimset(  'LargeScale','off','GradObj','on', 'TolFun',1e-5, 'TolX',1e-5, 'TolCon', 1e-5, 'MaxIter',  config.max_iter);%, 'display', 'iter');
end
options = optimset(options, 'outputfcn',@outfun);
%options = optimset(options, 'MaxFunEvals', config.max_iter, 'MaxIter', config.max_iter, 'display', 'iter');
     ...
     switch state
         ...
         case 'iter'
               ...
               if config.display>0
                   ...
                   switch lower(config.functionType)
                       case 'tps'
                           transformed = mg_transform_tps_parallel(x, config.model, config.ctrl_pts);
                       otherwise
